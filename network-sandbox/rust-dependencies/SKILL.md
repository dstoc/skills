...
description: Manage Rust/Cargo dependencies in this environment. Use when adding, updating, or fetching crates, editing `Cargo.toml`, or working within the constraint that the only approved network command is `cargo fetch` via `run-with-network`.
---
...
- Assume the only allowed network command is `cargo fetch` and it must be run through `run-with-network`.
- Always include the `--` delimiter between `run-with-network` options and the command.
